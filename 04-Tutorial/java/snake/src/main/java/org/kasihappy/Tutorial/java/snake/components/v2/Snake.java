...
public class Snake {
    ...
    private Location adjustHeadLocation()
    {
        ...
        switch (direction) {
            case NORTH:
                ty = ty - snakeUnit;
                if (ty < GameJPanel_v2.PANEL_TIPS)
                {
                    ty = GameJPanel_v2.PANEL_HEIGHT + GameJPanel_v2.PANEL_TIPS;
                }
                break;
            case SOUTH:
                ty = ty + snakeUnit;
                if (ty > GameJPanel_v2.PANEL_HEIGHT + GameJPanel_v2.PANEL_TIPS)
                {
                    ty = GameJPanel_v2.PANEL_TIPS;
                }
                break;
            case WEST:
                tx = tx - snakeUnit;
                if(tx < 0)
                {
                    tx = GameJPanel_v2.PANEL_WIDTH;
                }
                break;
            case EAST:
                tx = tx + snakeUnit;
                if (tx > GameJPanel_v2.PANEL_WIDTH)
                {
                    tx = snakeUnit;
                }
                break;
